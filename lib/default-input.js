...
if (!package.repository) {
  exports.repository = function (cb) {
    fs.readFile('.git/config', 'utf8', function (er, gconf) {
      if (er || !gconf) {
        return cb(null, yes ? '' : prompt('git repository', 'git@github.com:' + config.get('github_organisation') + '/' + exports.name + '.git'))
      }
      gconf = gconf.split(/\r?\n/)
...
exports.brain = function(cb) {
    var res = package.brain || {};
    if (!package.brain || !package.brain.provides) {
        res.provides = yes ? '' : prompt('provides knowledge', function (s) {
            if (!s) return undefined
            if (Array.isArray(s)) s = s.join(' ')
            if (typeof s !== 'string') return s
            return s.split(/[\s,]+/)
        ...
        res.provides = package.brain.provides;
    }
    if (!package.brain || !package.brain.requires) {
        res.requires = yes ? '' : prompt('requires knowledge', function (s) {
            if (!s) return undefined
            if (Array.isArray(s)) s = s.join(' ')
            if (typeof s !== 'string') return s
            return s.split(/[\s,]+/)
        ...
        res.requires = package.brain.requires;
    }
    if (!package.brain || !package.brain.track) {
        res.track = prompt('track [blue, black, yellow]', 'yellow');
    } else {
        res.track = package.brain.track;
    }
    cb(null, res);
};
